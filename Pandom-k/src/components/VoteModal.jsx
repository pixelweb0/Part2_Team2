...
import { CheckIcon, GuideLine, Highlight, IdolImgProfile, IdolImgWrap, IdolInfo, IdolItem, IdolListContent, ModalTitle, Name, RadioCircle, RankAndImg, RankNumber, SelectedOverlay, ShortageImg, VoteAmount, VoteButton } from "./VoteModal.styled";
  ...
  return (
        <BaseModal isOpen={isOpen} onClose={onClose} $width={modalSize[ModalStep].width}>
          ...
          <>
            ...
            <IdolListContent>
              {idols.map((idol, index) => (
                <IdolItem key={idol.id} onClick={() => setSelectedIdol(idol.id)}>
                  ...
                  <IdolInfo>
                    <Name>{idol.group} {idol.name}</Name>
            ...
            <VoteButton disabled={!selectedIdol} onClick={handleVote}>투표하기</VoteButton>
            <GuideLine>투표 시 <Highlight>1000 크레딧</Highlight>이 소모됩니다.</GuideLine>
          </>
          )}
          {ModalStep === 'success' && (
            <>
          ...
          {ModalStep === 'shortage' && (
            <>
              <ShortageImg src={CreditImg} />
              <p>앗! 투표하기 위한 <Highlight>크레딧</Highlight>이 부족해요</p>
              <VoteButton onClick={handleClose}>확인</VoteButton>
            </>
          )}
        </BaseModal>
